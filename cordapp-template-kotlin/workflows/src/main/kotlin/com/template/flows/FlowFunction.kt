...
abstract class FlowFunction : FlowLogic<SignedTransaction>() {
    ...
    fun inputStateRef(linearId: UniqueIdentifier): StateAndRef<airplaneState> {
        val criteria = QueryCriteria.LinearStateQueryCriteria(linearId = listOf(linearId))
        return serviceHub.vaultService.queryBy<airplaneState>(criteria).states.single()
    }

    fun iState(linearId: UniqueIdentifier): StateAndRef<airplaneState> {
        val criteria = QueryCriteria.LinearStateQueryCriteria(linearId = listOf(linearId))
        return serviceHub.vaultService.queryBy<airplaneState>(criteria).states.single()
    }
    ...
    fun inputStateAndRef(id: String): StateAndRef<airplaneState> {
        val queryCriteria = QueryCriteria.LinearStateQueryCriteria(linearId = listOf(stringToUniqueIdentifier(id)))
        return serviceHub.vaultService.queryBy<airplaneState>(queryCriteria).states.single()
    }
    ...
    fun getTime(id: UniqueIdentifier): Instant {
        ...
        val results = serviceHub.vaultService.queryBy<airplaneState>(queryCriteria)
